...
INPUT3 = """bvwbjplbgvbhsrlpgdmjqwftvncz"""
INPUT4 = """nppdvjthqldpwncqszvftbrmjlhg"""
INPUT5 = """nznrnfrfntjfmvfwmzdfjlvtqnbhcprsg"""
INPUT6 = """zcfzfwzzqfrljwzlrfnpqdbhtmscgvjw"""


def find_index_of_marker(s):
    size = 4
    for i in range(len(s)):
        section = s[i : i + size]
        if len(set(section)) == size:
            return i + size
    return


def find_index_of_msg(s):
    size = 14
    for i in range(len(s)):
        section = s[i : i + size]
        if len(set(section)) == size:
...
def part1():
    ...
    the Elves, the start of a packet is indicated by a sequence of four characters
    ...
    assert (val := find_index_of_marker(INPUT2)) == 7, f"got {val}"
    assert (val := find_index_of_marker(INPUT3)) == 5, f"got {val}"
    assert (val := find_index_of_marker(INPUT4)) == 6, f"got {val}"
    assert (val := find_index_of_marker(INPUT5)) == 10, f"got {val}"
    assert (val := find_index_of_marker(INPUT6)) == 11, f"got {val}"

    print(f"{find_index_of_marker(INPUT)=}")
...
def part2():
    ...
    assert (val := find_index_of_msg(INPUT2)) == 19, f"got {val}"
    assert (val := find_index_of_msg(INPUT3)) == 23, f"got {val}"
    assert (val := find_index_of_msg(INPUT4)) == 23, f"got {val}"
    assert (val := find_index_of_msg(INPUT5)) == 29, f"got {val}"
    assert (val := find_index_of_msg(INPUT6)) == 26, f"got {val}"
    print(f"{find_index_of_msg(INPUT)=}")
    return
